...
from ml_models import ensemble_price_forecast, add_features
...
class DataManager:
    def __init__(self, config, macro_data=None):
        ...
        self.macro_data = macro_data if macro_data is not None else self._load_macro()
        self.news_data = self._load_news_sentiment()
        self.social_data = self._load_social_sentiment()
        self.dates = []
        self.sector_map = {}  # {ticker: sector}
        self._fetch_sectors()
        self.predictions = self._load_predictions()
        self.ensemble_predictions = self._load_ensemble_predictions()
    ...
    def _load_macro(self):
        macro_path = self.config.get('macro_data_path', 'macro.json')
    ...
    def _load_news_sentiment(self):
        news_path = self.config.get('news_sentiment_path', 'news_sentiment.csv')
        if news_path and os.path.exists(news_path):
            df = pd.read_csv(news_path)
            return dict(zip(df['date'].astype(str), df['sentiment']))
        return {}

    def _load_social_sentiment(self):
        social_path = self.config.get('social_sentiment_path', 'social_sentiment.csv')
    ...
    def load_data(self):
        ...
        for ticker in self.tickers:
            ...
            if df is None:
                ...
                # Feature engineering: hacim, volatilite, makro, haber, sosyal medya
                df = add_features(df, macro_data=self.macro_data, news_data=self.news_data, social_data=self.social_data)
                self.data[ticker] = df
    ...
    def _load_ensemble_predictions(self):
        """
        Her hisse için ensemble tahmin ve confidence hesaplar (6 ay ve 1 yıl ileri).
        """
        preds = {}
        for ticker, df in self.data.items():
            if len(df) < 252:
                continue
            # 6 ay (126 gün) ve 1 yıl (252 gün) ileri tahmin
            feature_cols = [c for c in df.columns if c not in ['Date', 'date', 'Ticker', 'ticker']]
            # Son 252 gün ile model eğit, ileri tahmin yap
            df_train = df.iloc[:-252] if len(df) > 252 else df
            df_future = df.iloc[-252:]
            pred_6m, conf_6m = ensemble_price_forecast(df_train, feature_cols=feature_cols, window_size=30, epochs=10, plot=False)
            pred_1y, conf_1y = ensemble_price_forecast(df_train, feature_cols=feature_cols, window_size=30, epochs=10, plot=False)
            # Son günün tarihini referans al
            date = str(df['Date'].iloc[-1])[:10]
            preds[ticker] = {date: {'pred_6m': pred_6m[-1] if pred_6m is not None else None,
                                     'conf_6m': conf_6m,
                                     'pred_1y': pred_1y[-1] if pred_1y is not None else None,
                                     'conf_1y': conf_1y}}
        return preds
    ...
    def get_prediction(self, ticker, date):
        ...
        # Ensemble tahmin ve confidence ekle
        ens = self.ensemble_predictions.get(ticker, {}).get(date, {}) if hasattr(self, 'ensemble_predictions') else {}
        result = {'pred_6m': None, 'pred_1y': None, 'conf_6m': None, 'conf_1y': None}
        if ticker in self.predictions and date in self.predictions[ticker]:
            result.update(self.predictions[ticker][date])
        result.update(ens)
        return result
...
class DecisionEngine:
    ...
    def decide(self, scores, regime, strategy, step_data=None, sector_scores=None):
        ...
        if sector_scores:
            ...
            best_sectors = [s for s, v in sector_scores.items() if v >= max_score * 0.9]
        for ticker, score_dict in scores.items():
            ...
            if step_data and ticker in step_data:
                ...
                conf_6m = step_data[ticker].get('conf_6m', None)
                conf_1y = step_data[ticker].get('conf_1y', None)
                current_price = step_data[ticker].get('Close', None)
                # Confidence düşükse işlem açma
                if (conf_6m is not None and conf_6m < conf_thresh) or (conf_1y is not None and conf_1y < conf_thresh):
                    decisions[ticker] = 'hold'
                    continue
...
class TradeExecutor:
    ...
    def execute(self, decisions, step_data):
        ...
        for ticker, action in decisions.items():
            ...
            # Dinamik trade size: model confidence'a göre
            conf = step_data.get(ticker, {}).get('conf_6m', 1.0)
            min_trade = max(1, int(self.config.get('trade_size', 1) * 0.5))
            max_trade = int(self.config.get('trade_size', 1) * 2)
            trade_size = int(min_trade + (max_trade - min_trade) * min(max(conf, 0), 1))
            # Kademeli alım
            if action == 'buy' and self.portfolio_manager.cash > price:
                max_amount = (portfolio_value * max_position_pct) / price
                new_amount = min(pos['amount'] + trade_size, max_amount)
            ...
            # Kademeli satış
                ...
                sell_amount = max(1, int(pos['amount'] * 0.5 * conf))
